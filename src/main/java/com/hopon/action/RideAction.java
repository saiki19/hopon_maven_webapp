...
public class RideAction extends BaseAction {
	/*
	 * protected boolean recurring = false; protected boolean
	 * allowRecurringSubRideToCancel = true; protected RideManagementDTO
	 * rideRegistered = new RideManagementDTO(); protected List<RideSeekerDTO>
	 * allSeekerForGivenRide = new ArrayList<RideSeekerDTO>(); protected
	 * FrequencyDTO frequencyDTO = new FrequencyDTO(); protected
	 * List<RideManagementDTO> rideManagementList = new
	 * ArrayList<RideManagementDTO>(); protected List<RideSeekerDTO>
	 * rideSeekerList = new ArrayList<RideSeekerDTO>(); protected
	 * List<RideSeekerDTO> recurringRideSeekerList = new
	 * ArrayList<RideSeekerDTO>(); protected List<PoolRequestsDTO>
	 * allCompleateRideList = new ArrayList<PoolRequestsDTO>(); protected
	 * List<PoolRequestsDTO> allCompleateRideSeekerList = new
	 * ArrayList<PoolRequestsDTO>(); protected List<PoolRequestsDTO>
	 * allMsgBoardCompleateRideList = new ArrayList<PoolRequestsDTO>();
	 * protected List<PoolRequestsDTO> allMsgBoardCompleateRideSeekerList = new
	 * ArrayList<PoolRequestsDTO>(); protected ApproverDTO approverdto = new
	 * ApproverDTO(); protected List<ApproverDTO> approverdtoList = new
	 * ArrayList<ApproverDTO>(); protected PoolRequestsDTO poolRequestsDTO = new
	 * PoolRequestsDTO(); protected List<MatchedTripDTO>
	 * matchedTripByConditionList = new ArrayList<MatchedTripDTO>(); protected
	 * List<CombineRideDTO> combineVehicleCondition = new
	 * ArrayList<CombineRideDTO>(); protected List<RideSeekerDTO>
	 * allRideApprovalRequest = new ArrayList<RideSeekerDTO>(); protected
	 * MatchedTripDataModel matchedTripDataModel = new MatchedTripDataModel();
	 * protected CombineVehicleDataModel combineVehicleDataModel = new
	 * CombineVehicleDataModel(); protected MatchedTripDTO[] matchedTripDTOs ;
	 * protected CombineRideDTO[] combineVehicleDtos; protected RideSeekerDTO
	 * rideSeekerDTO = new RideSeekerDTO(); protected RideManagementDTO
	 * rideManagementDTO = new RideManagementDTO(); protected int rideIdToDrop;
	 * protected int rideIdToTake; protected String rideManager = "takeRide";
	 * protected int ridePicker = 1; protected List<RideManagementDTO>
	 * rideManagementListForPopupOne = new ArrayList<RideManagementDTO>();
	 * protected MatchedTripDTO matchedTripDTO = new MatchedTripDTO(); protected
	 * boolean pendingRideTest; protected boolean ridePreMatchFormTest = false;
	 * protected boolean rideMatchFormTest = false; protected ManageRideFormDTO
	 * manageRideFormDTO = new ManageRideFormDTO(); protected
	 * List<ManageRideDTO> manageRideDTOs = new ArrayList<ManageRideDTO>();
	 * protected String matchRideCancelParam; protected int ridePreMatchTest;
	 * protected RideManagementDTO rideManagerInfoForRideSeeker = new
	 * RideManagementDTO();
	 */
					...
					try {
						windoqCalculation = ApplicationUtil
								.calculateTimeWindowSettings(
										...
										userPreferences.getMaxWaitTime(),
										rideRegistered.getStartdateValue());
						System.out.println("window calculation is:"
								+ windoqCalculation);
		...
		if (!test) {
			...
			try {
				for (int i = 0; i < matchedTripDTOs.length; i++) {
					...
					// RideManagementDTO managementDtoTemp2 =
					// ListOfValuesManager.getRideManagerPopupData(dto.getSeekerID());
					System.out.println("SeekerDtoTemp get the rideseekerdata:"
							+ seekerDtoTemp);
					poolDTO.setRidemanagerID(rideManagementDTO.getRideID());
					poolDTO.setRideSeekerID(dto.getSeekerID());
					poolDTO.setPoolRequestToId(Integer
					...
					dtoSeeker = ListOfValuesManager.changeField(dtoSeeker, con);
					System.out
							.println("DtoSeeker Object is Printing in the RideAction:"
									+ dtoSeeker);
				}
			} catch (ConfigurationException e) {
				LoggerSingleton.getInstance().error(
	...
	public void populateApprover() {
		...
		if (currentSession.getAttribute("approverdtoList") != null) {
			approverdtoList.addAll((List<ApproverDTO>) currentSession
					.getAttribute("approverdtoList"));
		} else {
			try {
				approverdtoList.addAll(ListOfValuesManager
						.findApproverForUser(Integer
								.parseInt(userRegistrationDTO.getId())));
