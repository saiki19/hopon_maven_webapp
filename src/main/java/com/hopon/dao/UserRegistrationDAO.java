...
public class UserRegistrationDAO {
	public UserRegistrationDTO registerUser(Connection con,
		...
		pstmt.setString(8, ApplicationUtil.passwordEncrypt(userRegistrationDTO
				.getPassword()));
		...
		if (userRegistrationDTO.getDefaultTimeSlice() == null
				|| userRegistrationDTO.getDefaultTimeSlice().length() == 0) {
	...
	public UserRegistrationDTO closeUserAcount(Connection con,
			UserRegistrationDTO userDTO, String senderId) throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("UPDATE users set status = 'I', updated_by = '" + senderId
				+ "', updated_dt = '" + ApplicationUtil.currentTimeStamp()
				+ "' WHERE id = '" + userDTO.getId() + "'");// status 2->I
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		userDTO.setStatus("I");
		pstmt.executeUpdate();
		pstmt.close();
	...
	public UserRegistrationDTO confirmOrDeclineUser(Connection con,
			UserRegistrationDTO userdto, String senderId) throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append("UPDATE users SET  status = '" + userdto.getStatus()
				+ "', updated_by = '" + senderId + "', updated_dt = '"
				+ ApplicationUtil.currentTimeStamp() + "' where id ='"
				+ userdto.getId() + "'");
	...
	public UserRegistrationDTO updateUserPassword(Connection con,
			UserRegistrationDTO userRegistrationDTO, String senderId)
			throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("UPDATE users SET password= '"
				+ ApplicationUtil.passwordEncrypt(userRegistrationDTO
						.getPassword()) + "', updated_by = '" + senderId
				+ "', updated_dt = '" + ApplicationUtil.currentTimeStamp()
				+ "' where id ='" + userRegistrationDTO.getId() + "'");
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		pstmt.executeUpdate();
		pstmt.close();
		return userRegistrationDTO;
	...
	public UserRegistrationDTO updateUser(Connection con,
			UserRegistrationDTO userRegistrationDTO, String senderId)
			throws SQLException {
		if (userRegistrationDTO.getGender().equalsIgnoreCase("m")) {
			userRegistrationDTO.setGender("male");
		...
		StringBuilder query = new StringBuilder();
		query.append("UPDATE users SET first_name = '"
				...
				+ ApplicationUtil.passwordEncrypt(userRegistrationDTO
						.getPassword()) + "'" + ",travel= '"
				+ userRegistrationDTO.getTravel() + "',status= 'A',city= '"
				+ userRegistrationDTO.getCityId() + "',pincode='"
				+ userRegistrationDTO.getPincode() + "', defaultTimeSlice = ?"
	...
	public List<UserRegistrationDTO> getAllUser(Connection con, String prefix,
		...
		if (prefix == null || prefix.equals("")) {
			...
			query.append(" or last_name like '%" + prefix
	...
	public boolean testUniqueMobileNumber(Connection con, String mobileNumber,
			int excludedUserId) throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("Select count(*) from users where mobile_no = '"
				+ mobileNumber + "' ");
		if (excludedUserId > 0)
			query.append(" AND users.id != '" + excludedUserId + "'");
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		boolean test = false;
		while (rs.next()) {
	...
	public boolean testEmail(Connection con, String email) throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("Select count(*) from users where email_id = '" + email
				+ "' AND status = 'A'");
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		boolean test = false;
	...
	public UserRegistrationDTO UpdateRandomPassword(Connection con,
			String userName, String randomPassword, String senderId)
			throws SQLException {
		UserRegistrationDTO userdto = new UserRegistrationDTO();
		StringBuilder query = new StringBuilder();
		query.append("UPDATE users SET  password = '"
				+ ApplicationUtil.passwordEncrypt(randomPassword)
				+ "', updated_by = '" + senderId + "', updated_dt = '"
				+ ApplicationUtil.currentTimeStamp() + "' where email_id ='"
				+ userName + "'");
		...
		query.append("Select u.id,u.first_name,u.email_id,u.password,u.travel,u.address,u.mobile_no, u.birthdate,u.gender,u.city,u.pincode,u.last_name,u.defaultTimeSlice, t.city_name from users u LEFT OUTER JOIN tbl_citylist t ON t.city_id = u.city where u.email_id = '"
				+ userName + "'");
	...
	public List<UserRegistrationDTO> getAllUserById(Connection con,
			List<Integer> userId) throws SQLException {
		StringBuilder query = new StringBuilder();
		String inCondition = "";
		for (int i : userId) {
			inCondition += i + ",";
		}
		query.append("Select ID,email_id, first_name, mobile_no, last_name, address, gender, status from users where id IN ("
				+ inCondition + "0)");
	...
	public UserRegistrationDTO getUserById(Connection con, int userId)
			throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("Select u.ID,u.email_id, u.first_name, u.mobile_no, u.last_name, u.address, u.gender, u.city, u.pincode, u.state, u.country, t.city_name, u.latitude, u.longitude from users u LEFT OUTER JOIN tbl_citylist t ON u.city = t.city_id where id = '"
				+ userId + "'");
	...
	public List<UserRegistrationDTO> allUserOfAcircle(Connection con,
			String userID) throws SQLException {
		List<UserRegistrationDTO> allUser = new ArrayList<UserRegistrationDTO>();
		StringBuilder query = new StringBuilder();
		query.append("select users.id , users.first_name,users.address from users, circle_members,circles where circles.CircleOwner_Member_Id_P='"
				+ userID
				+ "' and circles.Circle_Id = circle_members.CircleId and circle_members.MemberId = users.id and circle_members.Status = 1 AND users.`status` = 'A'");
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		while (rs.next()) {
			UserRegistrationDTO dto = new UserRegistrationDTO();
	...
	public UserRegistrationDTO findUserByEmail(Connection con, String emailId)
		...
		UserRegistrationDTO dto = new UserRegistrationDTO();
		StringBuilder query = new StringBuilder();
		query.append("select id, email_id, mobile_no, gender, birthdate, address from users WHERE email_id = '"
				+ emailId + "' AND status = 'A'");
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		if (rs.next()) {
			dto.setId(rs.getString(1));
	...
	public boolean verifyUser(Connection con, UserRegistrationDTO dto)
			throws SQLException {
		StringBuilder query = new StringBuilder();
	...
	public void updateTotalCreditById(Connection con, int userId, float amount,
	...
	public UserRegistrationDTO fetchTravelType(Connection con,String id) {
		UserRegistrationDTO dto = new UserRegistrationDTO();
		StringBuilder query = new StringBuilder();
		query.append("SELECT travel FROM users WHERE id='" +id+"'");
	...
	public GuestRideDTO insertGuestInfo(Connection con, GuestRideDTO dto)
			...
			try {
				...
				pstmt.setString(1, dto.getGuest_id());
				pstmt.setString(2, dto.getGuest_fname());
				pstmt.setString(3, dto.getGuest_lname());
				...
				rs.close();
				pstmt.close();
				dto.setGuest_id(String.valueOf(autoGeneratedID));
				
				//Another way of Getting AutoGenerated Key
				/*	if(rs!=null){
				if(rs.first()){
					int newId=rs.getInt(1);
					dto.setGuest_id(String.valueOf(newId));
				}
			}*/
			} catch (SQLException e) {
				e.printStackTrace();
