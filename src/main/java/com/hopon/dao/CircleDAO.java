...
public class CircleDAO {
	public List<CircleDTO> findAllCircle(Connection con, final String userid)
			throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append(" SELECT  Circle_Id,  Name, Description,Date_of_creation,CircleOwner_Member_Id_P,AutoEnroll_YN,Affiliations, circle_type FROM circles where CircleOwner_Member_Id_P = '"
				+ userid + "' and  status='1' ");
	...
	public List<CircleDTO> findAllRegisteredCircle(Connection con)
			throws SQLException {
		...
		while (rs.next()) {
			...
			if (rs.getString(7) != null
					&& rs.getString(7).equalsIgnoreCase("T")) {
	...
	public List<CircleDTO> findAllStatusRegisteredCircle(Connection con)
			throws SQLException {
	...
	public List<CircleDTO> findAllRegisteredNonTaxiCircle(Connection con,
			String prefix) throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append(" SELECT  circles.Circle_Id,  circles.Name, circles.Description,circles.CircleOwner_Member_Id_P,circles.AutoEnroll_YN,circles.Affiliations, users.travel, circles.circle_type FROM circles left OUTER JOIN users ON circles.CircleOwner_Member_Id_P = users.id WHERE circles.status='1' AND users.travel != 'T' AND circles.Name like '%"
				+ prefix + "%'");
	...
	public List<CircleDTO> findAllRegisteredTaxiCircle(Connection con,
			String prefix) throws SQLException {
		System.out.println("RegistaerTaxiCircle By Name:" + prefix);
		StringBuilder query = new StringBuilder();
		query.append(" SELECT  circles.Circle_Id,  circles.Name, circles.Description,circles.CircleOwner_Member_Id_P,circles.AutoEnroll_YN,circles.Affiliations, users.travel, circles.circle_type FROM circles left OUTER JOIN users ON circles.CircleOwner_Member_Id_P = users.id WHERE circles.status='1' AND users.travel = 'T' AND circles.Name like '%"
				+ prefix + "%'");
	...
	public List<CircleDTO> findAllRegisteredCorpCircle(Connection con)
			throws SQLException {
	...
	public CircleDTO registerCircle(Connection con, CircleDTO circleDTO)
			throws SQLException {
		...
		pstmt.setString(5,
				String.valueOf(circleDTO.getCircleOwner_Member_Id_P()));
	...
	public CircleDTO declineCircle(Connection con, CircleDTO circleDTO,
			String userId) throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append("UPDATE circles SET  status = '2' where Circle_Id ='"
				+ circleDTO.getCircleID()
				+ "' AND CircleOwner_Member_Id_P != '" + userId + "'");
	...
	public CircleDTO activateCircle(Connection con, CircleDTO circleDTO)
			throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append("UPDATE circles SET  status = '1' where Circle_Id ='"
				+ circleDTO.getCircleID() + "' ");
	...
	public List<CircleDTO> findAllRegisteredCircleByName(Connection con,
			String circleName, String userId) throws SQLException {
		System.out.println("Circle Name:" + circleName + "User Id:" + userId);
		StringBuilder query = new StringBuilder();
		...
		query.append("select c1.Circle_Id,  c1.Name ,c1.Description,c1.Date_of_creation,c1.CircleOwner_Member_Id_P, c1.AutoEnroll_YN,c1.Affiliations, c2.Status, u.first_name, c1.circle_type FROM circles c1 left outer join circle_members c2 ON c1.Circle_Id = c2.CircleId AND c2.MemberId = '"
				+ userId
				+ "' INNER JOIN users u ON c1.CircleOwner_Member_Id_P = u.id WHERE c1.Name like '%"
				+ circleName + "%'");
...
/*
 * FindAllRegisteredTaxiCircleByName Method
 * 
 */
	public List<CircleDTO> findAllRegisteredTaxiCircleByName(Connection con,
			String circleName, String userId) throws SQLException {
		System.out.println("Circle Name:" + circleName + "User Id:" + userId);
		StringBuilder query = new StringBuilder();
		// query.append(" SELECT  Circle_Id,  Name, Description,Date_of_creation,CircleOwner_Member_Id_P,AutoEnroll_YN,Affiliations FROM circles where Name like'%"+circleName+"%'");
		...
		query.append("select c1.Circle_Id,  c1.Name ,c1.Description,c1.Date_of_creation,c1.CircleOwner_Member_Id_P, c1.AutoEnroll_YN,c1.Affiliations, c2.Status, u.first_name, c1.circle_type FROM circles c1 left outer join circle_members c2 ON c1.Circle_Id = c2.CircleId AND c2.MemberId = '"
				+ userId
				+ "' INNER JOIN users u ON c1.CircleOwner_Member_Id_P = u.id WHERE c1.circle_type='T' AND c1.Name like '%"
				+ circleName + "%'");
	...
	public List<CircleDTO> findAllRegisteredNonTaxiCircleByName(Connection con,
			String circleName, String userId) throws SQLException {
		System.out.println("Circle Name:" + circleName + "User Id:" + userId);
		StringBuilder query = new StringBuilder();
		// query.append(" SELECT  Circle_Id,  Name, Description,Date_of_creation,CircleOwner_Member_Id_P,AutoEnroll_YN,Affiliations FROM circles where Name like'%"+circleName+"%'");
	...
	public List<Integer> findAllUsersInCircle(Connection con, int circleId)
		...
		if (circleId == -1) {
			...
			query.append("select circle_members.MemberId from circle_members WHERE circle_members.Status = '1' AND circle_members.CircleId = "
					+ circleId);
		}
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
	...
	public List<Integer> findAllStatusUsersInCircle(Connection con, int circleId)
			throws SQLException {
		StringBuilder query = new StringBuilder();
		if (circleId == -1) {
			query.append("select circle_members.MemberId from circle_members WHERE 1 group by circle_members.MemberId");
		} else {
			query.append("select circle_members.MemberId from circle_members WHERE circle_members.CircleId = "
					+ circleId);
		}
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
	...
	public List<CircleDTO> findAllMemberCircle(Connection con, String userid)
			throws SQLException {
	...
	public List<CircleDTO> findAllPendingMemberCircle(Connection con,
			String userid) throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append(" SELECT  c.Circle_Id,  c.Name, c.Description, c.Date_of_creation, c.CircleOwner_Member_Id_P, u.first_name, c.circle_type FROM circles c LEFT OUTER JOIN users u ON c.CircleOwner_Member_Id_P = u.id where c.Circle_Id IN (select c2.CircleId from circle_members c2 where c2.MemberId = '"
				+ userid
				+ "' and c2.requestUserId != '"
				+ userid
				+ "' and c2.Status = '0') and  c.status='1' ");
	...
	public List<CircleDTO> findAllAffiliatedTaxiCircle(Connection con,
			String prefix) throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append(" SELECT  Circle_Id,  Name, Description,Date_of_creation,CircleOwner_Member_Id_P,AutoEnroll_YN,Affiliations, "
				+ "circle_type FROM circles where CircleOwner_Member_Id_P IN (select users.id from users where users.travel= 'T') "
				+ "and  status='1' and Name like '%" + prefix + "%' ");
	...
	public CircleDTO findCircleByCircleId(Connection con, int circleId)
			throws SQLException {
		CircleDTO dto = new CircleDTO();
		StringBuilder query = new StringBuilder();
		query.append("Select Circle_Id, Name, Description, Date_of_creation, CircleOwner_Member_Id_P, status, circle_type from circles where Circle_Id = "
				+ circleId);
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		while (rs.next()) {
	...
	public void makeCircleInactiveForUser(Connection con,
		...
		StringBuilder query = new StringBuilder();
		query.append("UPDATE circles SET status = '2' WHERE CircleOwner_Member_Id_P = "
	...
	public CircleDTO getCircleType(Connection con, int userId)
		...
		query.append("Select circle_type from circles where CircleOwner_Member_Id_P = "
				+ userId);
