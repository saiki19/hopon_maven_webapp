...
public class RideSeekerDAO {
	...
	public GuestRideDTO fetchGuestDetails(Connection con, MatchedTripDTO matchedTripDTO) {
		StringBuilder query = new StringBuilder();
		GuestRideDTO dto = new GuestRideDTO();
		query.append("SELECT first_name,last_name,mobile_no,email_id FROM guests WHERE id='"
				+matchedTripDTO.getGuest_id() + "'");
		try {
			PreparedStatement psmt = con.prepareStatement(query.toString());
			ResultSet rs = QueryExecuter.getResultSet(psmt, query.toString());
			while (rs.next()) {
	...
	public void findMaxRosterGroupId(Connection con, RideSeekerDTO rideDto) {
	    StringBuilder query2 = new StringBuilder();
		PreparedStatement pstmt2=null;
		query2.append("select REPLACE(group_id, 'RS-', '') + 1 as newGroupId, group_id from ride_seeker_details order by newGroupId + 0 desc limit 1");
		System.out.println("Max roster GroupId"+query2);
		try {
			pstmt2 = con.prepareStatement(query2.toString());
			ResultSet rs = QueryExecuter.getResultSet(pstmt2, query2.toString());
			while(rs.next()){		
				if(rs.getInt(1)!=0){
				String newRosterGroupId = "RS-"+rs.getInt(1);
				rideDto.setGroupId(newRosterGroupId);
				}
			}
			rs.close();
			//pstmt2.close();
	...
	public RideSeekerDTO rosterRideSeeker(Connection con ,RideSeekerDTO rideSeekerDTO )throws SQLException{
		StringBuilder query = new StringBuilder();
		query.append("INSERT INTO ride_seeker_details (seeker_id,user_id, start_point, via_point, destination_point," +
				"ride_cost,start_tw_early,status,FromCity," +
				"ToCity,FromPin,ToPin,created_by,created_dt, start_time, isSharedTaxi,custom, start_point_lat, " +
				"start_point_long, via_point_lat, via_point_long, end_point_lat, end_point_long, ride_distance, " +
	        	"start_tw_late, end_tw_early, end_tw_late, is_result, approverID, recurring, fullDay, circle_id, end_time, " + 
				" daily_rides, start_time2, group_id, trip_type ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
		...
		pstmt.setString(7, rideSeekerDTO.getStartdateValue());//getStartDate():date
		//pstmt.setString(8, rideSeekerDTO.getEnddateValue());
		...
		pstmt.setString(9, rideSeekerDTO.getFromAddressCity());
		pstmt.setString(10, rideSeekerDTO.getToAddressCity());
		pstmt.setString(11, rideSeekerDTO.getFromAddressPin());
		pstmt.setString(12, rideSeekerDTO.getToAddressPin());
		pstmt.setString(13, "1");
		if(rideSeekerDTO.getUpdatedDt()!=null){
			pstmt.setString(14, rideSeekerDTO.getUpdatedDt().toString());
		}
		else
		{
			pstmt.setString(14,null);	
		}
		pstmt.setString(15, rideSeekerDTO.getStartdateValue());
		if(rideSeekerDTO.isSharedTaxi()) {
			pstmt.setString(16, "1");
		} else {
			pstmt.setString(16, "0");
		}
		pstmt.setString(17, rideSeekerDTO.getCustom());
		...
		pstmt.setFloat(20, rideSeekerDTO.getViaPointLatitude());
		pstmt.setFloat(21, rideSeekerDTO.getViaPointLongitude());
		pstmt.setFloat(22, rideSeekerDTO.getEndPointLatitude());
		pstmt.setFloat(23, rideSeekerDTO.getEndPointLongitude());
		pstmt.setFloat(24, rideSeekerDTO.getRideDistance());
		pstmt.setString(25, rideSeekerDTO.getStartDateLate());
		pstmt.setString(26, rideSeekerDTO.getEndDateEarly());
		pstmt.setString(27, rideSeekerDTO.getEndDateLate());
		pstmt.setString(28, "N");
		pstmt.setInt(29, rideSeekerDTO.getApproverID());
		if(Validator.isEmpty(rideSeekerDTO.getRecurring())) rideSeekerDTO.setRecurring("Y");
		pstmt.setString(30, rideSeekerDTO.getRecurring());
		if(Validator.isEmpty(rideSeekerDTO.getFullDay())) rideSeekerDTO.setFullDay("N");
		pstmt.setString(31, rideSeekerDTO.getFullDay());
		pstmt.setInt(32, rideSeekerDTO.getCircleId());
		pstmt.setString(33, rideSeekerDTO.getEnddateValue());
		pstmt.setString(34, "Y");
		pstmt.setString(35, rideSeekerDTO.getStartTime2());
		pstmt.setString(36, rideSeekerDTO.getGroupId());
		pstmt.setInt(37, rideSeekerDTO.getTripType());
