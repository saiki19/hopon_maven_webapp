...
public class PoolRequestsDAO {
	public List<RideBillingReportDTO> gatherRideBillingData(
			final Connection con, final Date startDate, final Date endDate,
			int... circleId) throws SQLException {
		...
		if (subQuery.length() > 0)
			query.append(" AND c.Circle_Id IN(" + subQuery + "0) ");
		...
		while (rs.next()) {
			...
			try {
				dto.setRideDate(ApplicationUtil.dateFormat3.parse(rs
						.getString(13)));
				...
				LoggerSingleton.getInstance().error(
						e.getStackTrace()[0].getClassName() + "->"
								...
								+ "Date is : " + rs.getString(13) + "."
								+ e.getMessage());
			}
			try {
				dto.setRequestDate(ApplicationUtil.dateFormat5.parse(rs
						.getString(14)));
			} catch (ParseException e) {
				LoggerSingleton.getInstance().error(
						e.getStackTrace()[0].getClassName() + "->"
	...
	public List<SmsBillingReportDTO> gatherInboundSmsBillingData(
			final Connection con, final Date startDate, final Date endDate,
			int... circleId) throws SQLException {
		final List<SmsBillingReportDTO> list = new ArrayList<SmsBillingReportDTO>();
		StringBuilder query = new StringBuilder();
		query.append("select c.Circle_Id, c.Name, cou.id as ownerUserId, cou.email_id, cou.first_name, um.id, um.email_id, um.first_name, mb.MessageId, mu1.mobile_no, mb.CreatedbyDT from circles c LEFT OUTER JOIN users cou ON c.CircleOwner_Member_Id_P = cou.id LEFT OUTER JOIN circle_members cm ON c.Circle_Id = cm.CircleId LEFT OUTER JOIN users um ON cm.MemberId = um.id LEFT OUTER JOIN messageboard mb ON um.id = mb.CreatedBy LEFT OUTER JOIN users mu1 ON mb.CreatedBy = mu1.id WHERE mb.MessageChannel = 'S' AND mb.CreatedbyDT BETWEEN '"
				+ ApplicationUtil.dateFormat1.format(startDate)
				+ "' AND '"
				+ ApplicationUtil.dateFormat1.format(endDate) + "'");
		String subQuery = "";
		...
		while (rs.next()) {
			...
			try {
				dto.setSendDate(ApplicationUtil.dateFormat3.parse(rs
						.getString(11)));
			} catch (ParseException e) {
				LoggerSingleton.getInstance().error(
						e.getStackTrace()[0].getClassName() + "->"
	...
	public List<SmsBillingReportDTO> gatherOutboundSmsBillingData(
			final Connection con, final Date startDate, final Date endDate,
			int... circleId) throws SQLException {
		final List<SmsBillingReportDTO> list = new ArrayList<SmsBillingReportDTO>();
		StringBuilder query = new StringBuilder();
	...
	public List<PoolRequestsDTO> findAllPoolRequests(final Connection con,
		...
		query.append("SELECT  pr.id, rs.user_id,u1.first_name,u.travel,rm.start_point,rm.destination_point,rm.start_time,tf.Days,rt.ridegivernotes,rt.ridegiverrating, rt.ridetakernotes, rt.ridetakerrating, rm.`status` FROM pool_requests pr "
				...
				+
				// " where u.id = '"+userID+"' ");
				" where rm.status IN('A', 'T') AND u.id = '"
				+ userID
				+ "' AND TIMESTAMPDIFF(SECOND,rm.start_time,'"
				+ ApplicationUtil.currentTimeStamp() + "') > 0  "); // FOR
																	// UPDATE
				...
				try {
					...
					LoggerSingleton.getInstance().error(
							e.getStackTrace()[0].getClassName() + "->"
									...
									+ "() : "
									+ e.getStackTrace()[0].getLineNumber()
									+ " :: " + "Date is : " + rs.getString(7)
									+ "." + e.getMessage());
				}
			}
			dto.setRideManagementFrequency(rs.getString(8).substring(1,
					rs.getString(8).length() - 1));
	...
	public List<PoolRequestsDTO> findAllPoolRequestsSeekers(Connection con,
		...
		query.append("SELECT  pr.id, rm.user_id ,u1.first_name,u.travel,rs.start_point,rs.destination_point,rs.start_tw_early,tf.Days,rt.ridetakerrating,rt.ridetakernotes, rt.ridegivernotes, rt.ridegiverrating, u2.first_name FROM pool_requests pr "
				...
				+
				// " where u.id = '"+userID+"' ");
				" where rs.status = 'A' AND rs.is_result = 'Y' AND u.id = '"
				+ userID
				+ "' AND TIMESTAMPDIFF(SECOND,rs.start_tw_early,'"
				+ ApplicationUtil.currentTimeStamp() + "') > 0 ");// FOR UPDATE
		// " LEFT OUTER JOIN ratingandnotes rt ON rt.poolrequestid = pr.id INNER JOIN users u ON rm.user_id = u.id"
		// LEFT OUTER JOIN ratingandnotes rt ON rt.poolrequestid = pr.id INNER
		// JOIN users u ON rm.seeker_id = u.id
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		while (rs.next()) {
			PoolRequestsDTO dto = new PoolRequestsDTO();
			...
			dto.setRideSeekerFrequency(rs.getString(8).substring(1,
					rs.getString(8).length() - 1));
	...
	public PoolRequestsDTO updaterateAndWriteNotes(Connection con,
			PoolRequestsDTO pooldto) throws SQLException {

		StringBuilder query = new StringBuilder();
		// query.append("UPDATE pool_requests SET  ridetakerrating = '" +
		// pooldto.getRateRideTaker() + "', ridegiverrating = '" +
		// pooldto.getRateRideSeeker() + "',rideseekernotes = '" +
		// pooldto.getRideSeekerNotes() + "',ridetakernotes = '" +
		// pooldto.getRideTakerNotes() + "' where ID ='" + pooldto.getPoolID() +
		// "'");
	...
	public PoolRequestsDTO rateAndWriteNotes(Connection con,
			PoolRequestsDTO pooldto) throws SQLException {

		StringBuilder query = new StringBuilder();
		if (pooldto.getMasterControl().equalsIgnoreCase("giver")) {

			query.append("SELECT count(*) from ratingandnotes WHERE poolrequestid = "
					+ pooldto.getPoolID());
			PreparedStatement pstmt1 = con.prepareStatement(query.toString());
			ResultSet rs1 = QueryExecuter
					.getResultSet(pstmt1, query.toString());
			boolean test = true;
			while (rs1.next()) {
				if (rs1.getInt(1) > 0) {
					test = false;
			...
			if (test) {
				...
				if (pooldto.getRateRideGiver() == null
						|| pooldto.getRateRideGiver() == "") {
					pstmt.setString(3, "0");
				} else {
		...
		if (pooldto.getMasterControl().equalsIgnoreCase("taker")) {
			...
			if (test) {
				...
				pstmt.setString(1, pooldto.getRatingID());
				pstmt.setString(2, pooldto.getPoolID());
				if (pooldto.getRateRideTaker() == null
						|| pooldto.getRateRideTaker() == "") {
					pstmt.setString(3, "0");
				} else {
	...
	public UserRegistrationDTO getAverageRatingForUser(Connection con,
		...
		StringBuilder query = new StringBuilder();
		// query.append("select round(AVG(IFNULL(ratingandnotes.ridegiverrating, 0) + IFNULL(ratingandnotes.ridetakerrating, 0)), 1) as average_rating from ratingandnotes join pool_requests on ratingandnotes.poolrequestid = pool_requests.id join ride_seeker_details ON ride_seeker_details.seeker_id = pool_requests.rideseeker_id WHERE ride_seeker_details.user_id = '"+
		// dto.getId() +"'");
		query.append("select IFNULL(round(AVG(IFNULL(ratingandnotes.ridetakerrating, 0) ), 1), 0) as avg from ratingandnotes join pool_requests on ratingandnotes.poolrequestid = pool_requests.id LEFT OUTER JOIN rides_management r1 ON pool_requests.ride_id = r1.ride_id LEFT OUTER JOIN users u1 ON r1.user_id = u1.id WHERE u1.id = '"
				+ dto.getId()
				+ "'  AND ! ISNULL(ratingandnotes.ridetakerrating)");// FOR
																		// UPDATE
		/*
		 * select IFNULL(round(AVG(IFNULL(ratingandnotes.ridetakerrating, 0) ),
		 * 1), 0) as avg from ratingandnotes join pool_requests on
		 * ratingandnotes.poolrequestid = pool_requests.id LEFT OUTER JOIN
		 * rides_management r1 ON pool_requests.ride_id = r1.ride_id LEFT OUTER
		 * JOIN users u1 ON r1.user_id = u1.id LEFT OUTER JOIN
		 * ride_seeker_details r2 ON pool_requests.rideseeker_id = r2.seeker_id
		 * LEFT OUTER JOIN users u2 ON r2.user_id = u2.id WHERE u1.id =
		 * '"+dto.getId()+"'
		 */
		PreparedStatement pstmt = con.prepareStatement(query.toString());
		ResultSet rs = QueryExecuter.getResultSet(pstmt, query.toString());
		...
		// query.append("select round(AVG(IFNULL(ratingandnotes.ridegiverrating, 0) + IFNULL(ratingandnotes.ridetakerrating, 0)), 1) as average_rating from ratingandnotes join pool_requests on ratingandnotes.poolrequestid = pool_requests.id join ride_seeker_details ON ride_seeker_details.seeker_id = pool_requests.rideseeker_id WHERE ride_seeker_details.user_id = '"+
		// dto.getId() +"'");
		query.append("select IFNULL(round(AVG(IFNULL(ratingandnotes.ridegiverrating, 0) ), 1), 0) as avg from ratingandnotes join pool_requests on ratingandnotes.poolrequestid = pool_requests.id LEFT OUTER JOIN rides_management r1 ON pool_requests.ride_id = r1.ride_id LEFT OUTER JOIN users u1 ON r1.user_id = u1.id LEFT OUTER JOIN ride_seeker_details r2 ON pool_requests.rideseeker_id = r2.seeker_id LEFT OUTER JOIN users u2 ON r2.user_id = u2.id where u2.id = '"
				+ dto.getId()
				+ "' AND ! ISNULL(ratingandnotes.ridegiverrating)");
	...
	public PoolRequestsDTO insertInPool(Connection con, PoolRequestsDTO pooldto)
			throws SQLException {

		StringBuilder query = new StringBuilder();
		query.append("INSERT INTO pool_requests (id,ride_id,rideseeker_id, created_dt, request_time, request_status, user_id, ride_type, request_by, startRReplayresult, endRReplayresult) VALUES (?,?,?, '"
		...
		pstmt.setInt(5, pooldto.getPoolRequestToId());
		pstmt.setInt(6, pooldto.getRideType());
		pstmt.setInt(7, pooldto.getPoolRequestBy());
		pstmt.setString(8, "");
		pstmt.setString(9, "");
		pstmt.executeUpdate();
		ResultSet tableKeys = pstmt.getGeneratedKeys();
	...
	public void changePoolRequestStatusForRideTaker(Connection con,
			int rideSeekerId) throws SQLException {
		final StringBuilder query = new StringBuilder();
	...
	public void updateRideIdDropTake(Connection con,
		...
		query = new StringBuilder();
		query.append("update ride_seeker_details r set r.ride_match_rideid = ?, r.updated_dt = '"
				+ ApplicationUtil.currentTimeStamp()
				+ "' WHERE r.ride_match_rideid = ? AND TIMESTAMP(r.start_time) > '"
				+ ApplicationUtil.currentTimeStamp() + "' ");
		pstmt = con.prepareStatement(query.toString());
		pstmt.setString(1, rideToTake.getRideID());
		pstmt.setString(2, rideToDrop.getRideID());
	...
	public void fetchAllPreviousDayRides(Connection con) throws SQLException {
		StringBuilder query = new StringBuilder();
		...
		while (rs.next()) {
			...
			tempMap.put("requesterUserId", String.valueOf(requesterUserId));
			...
			if (totalValueMap.containsKey(rideId)
				...
				price += Double.parseDouble(totalValueMap.get(rideId).get(
						"cost"));
			}
		...
		/*
		 * Now transfer payment from hopon account to taxi admin account.
		 */
		...
		for (Entry<Integer, Map<String, String>> charges : totalValueMap
			...
			paymentTxnsDTO.setCreated_by(1);
			paymentTxnsDTO.setFrom_payer(100);
			paymentTxnsDTO.setTo_payee(Integer.parseInt(values
					.get("rideUserId")));
			paymentTxnsDTO.setTrip_details("");
			paymentTxnsDTO.setSeeker_id(0);
			paymentTxnsDTO.setCreated_dt(ApplicationUtil.currentTimeStamp());
			paymentTxnsDTO.setDistance(Float.parseFloat(String
					.valueOf(maxDistanceMap.get(rideId))));
			paymentTxnsDTO.setAmount(Float.parseFloat(values.get("cost")));
			...
			new UserRegistrationDAO().updateTotalCreditById(con,
					...
					-Float.parseFloat(values.get("cost")),"credit");
		}

		/*
		 * Now transfer payment to hopon account from ride seeker user account.
		 */
		...
		for (Entry<String, Map<String, String>> charges : rideSeekerChargeMap
			...
			PaymentTxnsDTO paymentTxnsDTO = new PaymentTxnsDTO();
			paymentTxnsDTO.setCreated_by(1);
			paymentTxnsDTO.setFrom_payer(Integer.parseInt(values
					.get("requesterUserId")));
			paymentTxnsDTO.setTo_payee(100);
			paymentTxnsDTO.setTrip_details("");
			paymentTxnsDTO.setSeeker_id(rideSeekerIdTemp);
			paymentTxnsDTO.setCreated_dt(ApplicationUtil.currentTimeStamp());
			paymentTxnsDTO
					.setDistance(Float.parseFloat(values.get("distance")));
			paymentTxnsDTO.setAmount(Float.parseFloat(values.get("cost")));
			// use any of one line.
			...
			new UserRegistrationDAO().updateTotalCreditById(con,
					...
					Float.parseFloat(values.get("cost")),"debit");
